...
export const listGenres = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("https://rejhinald.pythonanywhere.com/api/genres"); //fetch the products from rest api
...
export const addGenre = (genre) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post("https://rejhinald.pythonanywhere.com/api/genres/create", genre); //create a new product
